...
namespace System.Collections.Extern
{
    /// <summary>
    ...
    public class RingBuffer<T> : IEnumerable<T>, IEnumerable, ICollection<T>, 
        ...
        {
            if (pos >= size) 
                throw new System.InvalidOperationException("Peek Index is out of size.");
            int idx = (head + pos) % Capacity;
            T _item = buffer[idx];
            return _item;
        }
        //public T Popup(int pos)
        //{
        //    if (1 > size)
        //        return default(T);
        //    T v = buffer[head];
        ...
        //    head %= Capacity;
        //}
        public T[] Popup(int cnt)
        {
            if (cnt > size)
                return null;
            T[] data = new T[cnt];
            for (int i = 0; i < cnt; i++,head++)
            {
                head  %=  Capacity;
                data[i] = buffer[head];
            }
        ...
        public void Append(T[] items,int Length)
        {
            // If tail & head are equal and the buffer is not empty, assume
            // that it will overflow and throw an exception.
            if ( size + Length > Capacity)
            {
                throw new System.InvalidOperationException("The RingBuffer size is too small");
            }
            // If the buffer will not overflow, just add the item.
            else
            {
